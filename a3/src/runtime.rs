...
impl ServiceBuilder {
    ...
    pub fn build(self) -> Result<Service, ServiceBuildError> {
        ...
        Ok(Service {
            ...
            _auth_provider: self.auth_provider,
...
pub struct Service {
    ...
    pub version: String,
    endpoints: Vec<EndpointRegistration>,
    _auth_provider: Option<Arc<dyn AuthProvider>>,
    _rate_limiter: Arc<dyn RateLimitBackend>,
}
    ...
    pub fn into_router(self) -> Router {
        ...
        for ep in self.endpoints {
            router = match ep.method {
                HttpMethod::Get => router.route(&ep.path, ep.handler),
                HttpMethod::Post => router.route(&ep.path, ep.handler),
                HttpMethod::Put => router.route(&ep.path, ep.handler),
                HttpMethod::Patch => router.route(&ep.path, ep.handler),
                HttpMethod::Delete => router.route(&ep.path, ep.handler),
            };
        }
...
/// Middleware that injects a unique request ID into every response.
...
// Static header names for security headers (must be lowercase for HeaderName::from_static)
