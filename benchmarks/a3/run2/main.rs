//! User management service — CRUD operations with JWT auth and validation.
//!
...
//!     -d '{"username":"bob","email":"bob@example.com","display_name":"Bob"}'
...
async fn get_user(
    ...
    let user = users.get(&user_id).cloned().ok_or(UserError::NotFound)?;
    Ok(Json(user))
}
